...
Arranging data by publication year shows that the first known paper (according to Dimensions) with "Ten simple rules" in its title was published back in 1988, the same year Disney released *Who Framed Roger Rabbit?* In other words, Christopher Lloyd and M.C. Budden *et al*. were in a pioneering phase simultaneously.
...
a lot! About 500 of "10 simple rules" papers. That means in the scientific literature there are at least 5000 simple rules!
...
The large majority of these papers are coming from *PLOS Computational Biology*, and of course it's no coincidence that their Editor-in-Chief published the first modern 'Ten Simple Rules' paper back in 2005. Digging a bit further on Dimensions authorship data shows Philip is the king of simple rules.
...
PLOS Computational Biology APC is currently at \$2926, meaning each rule comes at a cost of \$296, and the entire series must have bring some serious money to the journal (APC changes overtime, but I´m estimating \>\$600k - assuming the series is treated as regular submissions.)
...
clean_data%>%
  ...
  ggplot(aes(x=PubYear,y=cit_per_year))+
  geom_jitter(height=0,width = .2,colour="darkred",size=3)+
  ...
  scale_x_continuous(breaks = seq(1988,2024,2))+
  labs(title = "The impact of 10 simple rules for....", y="Citations/year published",x="Year",caption = "Data source: Dimensions")+
  theme_economist()+
  theme(axis.text.y = element_text(hjust = 0),
        axis.title.y = element_text(vjust = 5),
        axis.text.x = element_text(angle=90))

ggsave("plot3.png",dpi="retina")
```
...
Stephen Heard had already discussed this trend on his blog less than a year ago. In addition to providing a clever parody of this popular genre of papers, his blog highlights what many consider the quintessential article in the series: "Ten Simple Rules for Writing a PLOS Ten Simple Rules Article.

[Stephen Heard](https://x.com/StephenBHeard) had already commented on this trend on his [blog](https://scientistseessquirrel.wordpress.com/2023/10/31/10-simple-rules-for-writing-10-simple-rules-papers/) less than a year ago. Along with a quality parody of this trending genre of papers, his blog ends up highlighso the epitome article of the *Ten simple rules*... series: [Ten Simple Rules for Writing a PLOS Ten Simple Rules Article](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1003858). Published back in 2014, the article is still a fun read with some interesting data on the series. Back then Philip E. Bourne was present in almost half of the papers on the series.

![2014 Pie chart from PLOS article shows Philip E. Bourne presence in the series](pcbi.1003858.g002.png)

Later, in 2018, Philip E. Bourne would then went into publishing [One thousand simple rules](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1006670). The paper ends rising some interesting questions about the series, its novelty and the future of science dissemination.

But here we are in 2024, and the series is still going strong, delivering ten rules at a time (several times a year).
